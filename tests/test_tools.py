...
def test_new_england_ranking_is_deterministic() -> None:
    ...
    assert {
        result.analysis.airport.state_code
...
def test_lax_and_santa_ana_comparison_works_without_an_llm() -> None:
    ...
    assert [
        analysis.airport.airport_code
        for analysis in result.airports
    ] == ["LAX", "SNA"]
    assert all(
        analysis.metrics.congestion_score is not None
        for analysis in result.airports
    )
...
def test_sfo_unmet_capacity_question_returns_formula_breakdown() -> None:
    ...
    assert "Supporting context only" in result.assumptions[2]
